...
// class App {
	...
	visualise_todoitem(todo_item) {
		...
		Later Summary:
		Within a class's constructor,
		
			document.addEventListener("app model changed", (event) => { this.notify(event) })
				the controller's notify function's 'this' will be the class instance, as expected in OO.
				*however cannot later unbind because have used anonymous function to which we have no reference
				 (not a problem if don't expect to unbind, e.g. for permanent wirings during the lifecycle of app/web page)

			document.addEventListener("modified todoitem", controller.notify)
				the controller's notify function's 'this' will be undefined - drats

				presumably because its called from the listener - cos in normal circumstances, calling a instance method
				from inside another instance method preserves the value of 'this'
				this loss of 'this' also happens when binding dom gui events and the same bind() trick is required e.g.
				.on('change', '.toggle', this.toggle.bind(this))

			document.addEventListener("modified todoitem", this.notify.bind(this))
				the controller's notify function's 'this' will be the class instance, as expected in OO.
				*however cannot later unbind because have used anonymous function (use of .bind() does this) to which we have no reference
